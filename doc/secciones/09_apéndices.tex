\chapter{Implementación detallada}
...
%     \item Un repositorio de github para el kérnel de FreeRTOS.
...
El repositorio del kérnel se encuentra en \path{FreeRTOS/Source} con respecto al repositorio principal.
...
Elegí basarme en el port \emph{ARM7\_LPC2000} porque es una familia de placas que tiene el mismo procesador que la MC13224V (ARM7TDMI-S) y solo tenía esos tres archivos, mientras que otros como el port \emph{ARM7\_AT91SAM7S} tiene otros archivos específicos del \emph{hardware}.\\
...
Las cabeceras que necesitamos están en este mismo directorio (\path{portmacro.h}), en el directorio de cabeceras de BSP y en el directorio de la demo (\emph{FreeRTOSConfig.h}).
...
En concreto mis mayores problemas fueron que el compilador no encontraba el BSP y que no encontraba el modo de modificar los \emph{flags} del enlazador.
...
\chapter{Código adjunto}
\section{Struct para configurar los temporizadores}
\label{chap:timer}
\lstset{
...
\begin{lstlisting}
struct tmr_regs_t{
    // Registro de comparacion utilizando cuando cuenta hacia arriba
    uint16_t COMP1 : 16; // 0x00 | TMRX_MASK
    // Registro de comparacion utilizando cuando cuenta hacia abajo
    ...
    // cargar en el comparador si se cumple una condicion
    ...
    /*PADDING 0x16 -> 0x1e */
    uint16_t                  : 8;
    // Regisro de Habilitacion del canales
    union { // ENBL : 0x1e
        struct {
            uint16_t ENBL0        : 1;
            uint16_t ENBL1        : 1;
